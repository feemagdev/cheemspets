...
import 'package:cheemspets/apis/api.dart';
import 'package:cheemspets/models/token_market_model.dart';
import 'package:cheemspets/models/tokens.dart';
...
class TokenService with ChangeNotifier {
  ...
  Future<void> _getMarketData(List<_TokensID> tokenID,
      SharedPreferences sharedPreferences, String idString) async {
    tokenMarketData.clear();
    totaCurrenctlValue = 0.0;
        ...
        if (element2.id == 'CHEEMSPETS') {
          ...
          if (sharedPreferences.containsKey(marketModel.symbol)) {
            if (sharedPreferences.getBool(marketModel.symbol)!) {
              totaCurrenctlValue += double.parse(element2.balance) /
  ...
  Future<List<_TokensID>> getTokensID(List<Token> tokens) async {
    String metaManAddress = '0x4ce41631013c1763d79b862a2889767a5fd54bc1';
  ...
  Future<void> getMetaManCoinInfo(String currency) async {
    ...
    final metaManInfoUrl =
        'https://api.nomics.com/v1/currencies/ticker?key=${ApiKeys.nomicsApi}&ids=CHEEMSPETS&convert=$currency&interval=interval=1h,1d,7d,30d,365d';
